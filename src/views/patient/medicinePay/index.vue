     ...
     <el-table
       ...
       >
            <template slot-scope="scope">
                <el-button
                size="mini"
                type="primary" @click="payDetailDialog=true">费用明细</el-button>
                <el-button
                size="mini"
                type="success" @click="showDialog">支付</el-button>
            </template>
        </el-table-column>
    </el-table>
    <el-dialog
...
export default {
  ...
  methods: {
      getMedicinePayList(){
          ...
          this.$store.dispatch('patient/getMedicinePayList', username ).then((data) => {
              if(data.resCode == 1){
     ...
     postPayRecord(){
        ...
        postPayRecord(postData).then(response => {
            Message({
                message: '支付成功',
                type: 'success',
                duration: 3 * 1000
            ...
            that.cancelSubmit()
            deleMedicalPay(that.row).then(response => {
